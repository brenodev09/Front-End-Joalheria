...
export default function ProdutoJoalheria() {
  ...
  function adicionarSacola() {
    ...
    setAdicionado(true)
    gsap.fromTo(botaoSacolaRef.current, { scale: 0.97 }, { scale: 1, duration: 0.5, ease: 'power2.out' })
          ...
          <div className={style.galeriaProduto} ref={galeriaRef}>
            ...
            >
              <img
                ...
                src={PRODUTO.imagens[indiceAtivo]}
                alt="Anel Solitário Constelação, ouro 18K com diamante natural"
                className={style.imagemPrincipal}
                style={
                  zoomAtivo && !modo360
                    ? { transformOrigin: origemZoom, transform: 'scale(1.9)' }
                    : undefined
                }
              />
            ...
            <div className={style.miniaturas}>
              {PRODUTO.imagens.map((imagem, indice) => (
                <button
                  key={imagem}
                  type="button"
                  className={`${style.miniatura} ${indice === indiceAtivo ? style.miniaturaAtiva : ''}`}
                  onClick={() => trocarImagem(indice)}
              ...
              <div className={`${style.gradeTamanhos} ${erroTamanho ? style.erroTamanho : ""}`}>
                {PRODUTO.tamanhos.map((tamanho) => (
                  <button
                    key={tamanho}
                    type="button"
                    className={`${style.opcaoTamanho} ${tamanho === tamanhoAtivo ? style.opcaoTamanhoAtiva : ''}`}
                    onClick={() => setTamanhoAtivo(tamanho)}
                  >
                    {tamanho}
                  </button>
                ))}
